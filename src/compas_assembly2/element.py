...
class Element(Data):
    ...
    def __init__(
        self, name=None, id=None, frame=None, geometry_simplified=None, geometry=None, insertion=None, **kwargs
    ):
        # --------------------------------------------------------------------------
        ...
        if geometry_simplified:
            if isinstance(geometry_simplified, list):
                ...
                if isinstance(geometry_simplified[0], (int, float)) and len(geometry_simplified) == 3:
                    self.geometry_simplified.append(
                        Point(geometry_simplified[0], geometry_simplified[1], geometry_simplified[2])
                    )
    ...
    def from_simplices_and_geometryes(cls, simplices=[], geometryes=None):
        ...
        for id, s in enumerate(simplices):
            if geometryes is list:
                elements.append(Element(geometry_simplified=s, geometry=geometryes[id % len(geometryes)]))
            else:
    ...
    def volume(self):
        ...
        # --------------------------------------------------------------------------
        vertex_index = {vertex: index for index, vertex in enumerate(self.geometry[0].vertices())}
        vertices = [self.geometry[0].vertex_coordinates(vertex) for vertex in self.geometry[0].vertices()]
        faces = [
    ...
    def centroid(self):
        ...
        # compute the centroid
        # --------------------------------------------------------------------------
        x, y, z = centroid_points([self.geometry[0].vertex_coordinates(key) for key in self.geometry[0].vertices()])
