...
class Algorithms:
    ...
    def get_collision_pairs_kdtree(
        elements, max_neighbours=10, check_element_collsion=True, attributes=[], skip_the_same=True
    ):
        # get points
        points = []
        diagonal_distances = []
        ...
        def find_nearest_neighbours(cloud, nmax, distance_list, dims=3, attributes=[], skip_the_same=True):
                                ...
                                if elements[sorted_idx].has_collision(elements[sorted_jdx]):
                                    if (
                                        ...
                                        or (
                                            are_attributes
                                            and skip_the_same
                                            and attributes[sorted_idx] != attributes[sorted_jdx]
                                        )
                                        or (
                                            are_attributes
        ...
        tuple_set = find_nearest_neighbours(points, max_neighbours, diagonal_distances, 3, attributes, skip_the_same)
    ...
    def shortest_path(model, element0, element1):
        ...
        if node_start_name in networkx_graph.nodes and node_end_name in networkx_graph.nodes:
            ...
            keys_of_elements = nx.shortest_path(
                networkx_graph, source=node_start_name, target=node_end_name, weight="weight"
            )
