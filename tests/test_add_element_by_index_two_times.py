...
@pytest.fixture
def setup_assemblies_merging_1():
    ...
    assembly_1.add_element_by_index(
        Element(id=[1, 3], frame=Frame.worldXY(), simplex=Point(0, 0, 0), attr={"t": "Block", "m": 30})
    )

    # assembly_1.add_element_by_index(
    #     Element(id=[2, 2, 3, 4], frame=Frame.worldXY(), simplex=Point(0, 0, 0), attr={"t": "Block", "m": 30})
    # )

    return assembly_1


def test_add_element_by_index_two_times(setup_assemblies_merging_1):
    assembly_0 = setup_assemblies_merging_1
